...
<body class="bg-gray-50">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p class="text-lg text-gray-600 mt-1">${quiz != null ? 'Cập nhật thông tin và quản lý câu trả lời' : 'Tạo câu hỏi trắc nghiệm mới cho khóa học của bạn'}</p>
                </div>
            </div>
        </div>
            ...
            <main class="w-full">
                ...
                <div class="bg-white rounded-lg shadow-sm mb-6">
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-600 mt-1">Điền thông tin bên dưới để ${quiz != null ? 'cập nhật' : 'tạo'} câu hỏi của bạn</p>
                    </div>

                    <div class="p-6">
                        <form id="quizForm" action="${pageContext.request.contextPath}/instructor/quizes" method="POST">
                                ...
                                <div>
                                    ...
                                    <select id="categoryId" name="categoryId" required
                                        ...
                                        <c:forEach var="cat" items="${categories}">
                                            <option value="${cat.id}" <c:if test="${quiz != null && cat.id == quiz.category_id}">selected</c:if>>${cat.name}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                            </div>
                ...
                <div id="answers-section" class="bg-white rounded-lg shadow-sm scroll-mt-4">
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                <h2 class="text-xl font-bold text-gray-900">
                                    <c:choose>
                                        <c:when test="${quiz != null}">Quản lý câu trả lời</c:when>
                                        <c:otherwise>Câu trả lời</c:otherwise>
                                    </c:choose>
                                </h2>
                                <p class="text-sm text-gray-600 mt-1">
                                    <c:choose>
                                        <c:when test="${quiz != null}">Thêm, chỉnh sửa hoặc xóa các câu trả lời cho câu hỏi này</c:when>
                                        <c:otherwise>Thêm các câu trả lời cho câu hỏi mới (Lưu câu hỏi trước để quản lý câu trả lời)</c:otherwise>
                                    </c:choose>
                                </p>
                            </div>
                            <c:if test="${quiz != null}">
                            ...
                            <c:choose>
                                    ...
                                    <div class="space-y-3">
                                        <c:forEach var="answer" items="${quizAnswers}">
                                            <div class="flex items-start gap-3 p-4 border-2 ${answer.is_true ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white'} rounded-lg hover:shadow-md transition"
                                                ...
                                                <div class="flex-grow">
                                                    <p class="text-base font-medium ${answer.is_true ? 'text-green-900' : 'text-gray-900'}">${answer.content}</p>
                                                    <p class="text-xs ${answer.is_true ? 'text-green-600' : 'text-gray-500'} mt-1">
                                                        ${answer.is_true ? '✓ Câu trả lời đúng' : 'Câu trả lời sai'}
                                                    </p>
                                                </div>
                                ...
                                <c:otherwise>
                                    <div class="text-center py-12">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900">Chưa có câu trả lời</h3>
                                        <p class="mt-1 text-sm text-gray-500">Bắt đầu bằng cách thêm câu trả lời đầu tiên cho câu hỏi này.</p>
                                        <div class="mt-6">
                                            <button type="button" onclick="showAddAnswerModal()"
                    ...
                    <c:if test="${quiz == null}">
                        <div class="p-12 text-center">
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900">Lưu câu hỏi trước</h3>
                            <p class="mt-2 text-sm text-gray-500 max-w-md mx-auto">
                                Vui lòng lưu câu hỏi trước, sau đó bạn có thể quay lại trang này để thêm, chỉnh sửa và quản lý các câu trả lời.
                            </p>
                            <div class="mt-6">
                                <button type="button" onclick="document.getElementById('quizForm').requestSubmit()"
    ...
    <div id="answerModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div class="flex items-center justify-between p-6 border-b border-gray-200">
                <h2 id="modalTitle" class="text-2xl font-bold text-gray-900">Thêm câu trả lời</h2>
                <button onclick="closeAnswerModal()" class="text-gray-400 hover:text-gray-600 transition">
            ...
            <form id="answerForm">
                ...
                <input type="hidden" id="quizIdInput" name="quizId" value="${quiz != null ? quiz.id : ''}">
                <input type="hidden" id="formAction" name="action">

                <div class="p-6 space-y-4">
                    <div>
                        <label for="answerContent" class="block text-sm font-semibold text-gray-700 mb-2">
